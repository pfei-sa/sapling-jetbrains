...
- **JCEF hygiene:** gate on `JcefSupport.isAvailable` (with a fallback panel) — **never call `JBCefApp.isSupported()` directly**: the JCEF classes can be absent from the process entirely (Rider / remote-dev backend), so resolving `JBCefApp` itself throws `NoClassDefFoundError` (issue #3). Wrap JCEF objects in a plugin-owned `Disposable`; register `JBCefJSQuery` as a child of the browser; guard late JS callbacks with a `@Volatile disposed` flag.
